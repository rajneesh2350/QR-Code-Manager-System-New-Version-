...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($logId > 0) {
        ...
        if (!$conn || $conn->connect_error) {
            error_log("Database connection failed in update_scan_log.php: " . ($conn->connect_error ?? 'No connection'));
        ...
        error_log("Invalid or missing log ID received: " . $logId);
        // Still return success even if no log ID (just don't update)
        echo json_encode(['success' => true, 'message' => 'No log ID provided, scan recorded but duration not updated']);
    }
} else {
